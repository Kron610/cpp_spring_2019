...
template<class T>
void mergeFiles(const std::string& filename1, const std::string& filename2, const std::string& filename_final,
                const size_t memory)
{
    const size_t type_size = sizeof(T);
    ...
    const auto chunk_number1 = static_cast<uint64_t>(ceilf(static_cast<float>(file_size1) / static_cast<float>(memory)));
    const auto chunk_number2 = static_cast<uint64_t>(ceilf(static_cast<float>(file_size2) / static_cast<float>(memory)));
    const size_t chunk_size = memory / type_size - (memory / type_size) % 2,
            chunk_byte_size = chunk_size * type_size;
...
std::string deleteZeroes(std::string filename, const size_t zeroes)
{
    ...
    else
    {
        ...
        while (!old.eof())
        {
            old.read(buffer, buffer_size);
            if (old.gcount())
                new_file.write(buffer, old.gcount());
        }
